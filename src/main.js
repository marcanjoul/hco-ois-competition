...
function enterAsDashboard(empId) {
  ...
  // Show persistent header and nav
...
function showScreen(name) {
  ...
  const titles = { dash: null, board: "LEADERBOARD", alltime: "ALL-TIME" };
  const headerName = document.getElementById("dash-name");
  const headerComp = document.getElementById("dash-comp-name");
  if (name !== "dash" && titles[name]) {
    headerName.textContent = titles[name];
    headerComp.textContent = name === "alltime" ? "Across all competitions" : (state.competitions[state.currentComp]?.name || "");
  } else if (name === "dash") {
    const emp = state.employees[state.currentUser];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Back button → go to pick screen, hide header/nav
  ...
  pickAdminBtn.onclick = (e) => {
    e.stopPropagation();
    pickMenu.classList.toggle("hidden");
  };

  // Admin open from pick screen
  document.getElementById("btn-pick-admin-open").onclick = () => {
    pickMenu.classList.add("hidden");
    openAdminModal();
  };

  // Admin open from in-app hamburger
